...
class BinaryTree {
public:
    BinaryTreeNode<T>* createWithPost(List<std::string>* str);
...
private:
    ...
    void PreOrderTraverse(BinaryTreeNode<T>* current);
    void InOrderTraverse(BinaryTreeNode<T>* current);
    void PostOrderTraverse(BinaryTreeNode<T>* current);
...
template<class T>
void BinaryTree<T>::InOrderTraverse(BinaryTreeNode<T>* current) {
    if (current != NULL) {
...
template<class T>
void  BinaryTree<T>::PreOrderTraverse(BinaryTreeNode<T>* current) {
    if (current != NULL) {
        std::cout << current->data << " ";
...
template<class T>
void BinaryTree<T>::PostOrderTraverse(BinaryTreeNode<T>* current) {
    if (current != NULL) {
...
template<class T>
BinaryTreeNode<T>* BinaryTree<T>::createWithPost(List<std::string>* str) {
    BinaryTreeNode<T>* ptr, * temp = new BinaryTreeNode<T>;
    LinkedStack<BinaryTreeNode<T>*>nodeStack;
    BinaryTreeNode<T>* useless;
    std::string c;
    
    int pos = 0;
    
    c = str->Find(pos++)->GetData();
    ...
    while (pos<=str->lenth())
    {
        ...
        else
        {
            ...
            {
                ptr->rightChild = nodeStack.getTop();
                nodeStack.Pop(useless);
            }
            if (!nodeStack.IsEmpty())
            {
                ptr->leftChild = nodeStack.getTop();
                nodeStack.Pop(useless);
            }
            nodeStack.Push(ptr);
            if (pos < str->lenth()) {
