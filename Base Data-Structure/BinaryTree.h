...
template<class T>
BinaryTree<T>::BinaryTree(BinaryTreeNode<T>*s){
    root=Copy(s);
}
...
template<class T>
BinaryTreeNode<T>* BinaryTree<T>::Find(T item)const{
    return Find(root,item);
}

template<class T>
BinaryTreeNode<T>* BinaryTree<T>::Find(BinaryTreeNode<T>* subTree, const T& item)const{ 
    if(subTree== NULL||subTree->data==item){
        return subTree;
    }
    else{
        BinaryTreeNode<T>*s=Find(subTree->leftChild,item);
        if(s== NULL){
            s=Find(subTree->rightChild,item);
        }
        return s; 
    }
}
